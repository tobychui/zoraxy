...
type UserResponse struct {
	ID             string   `json:"id"`             //User ID, generated by uuid, used for internal reference
	Username       string   `json:"username"`       //Username is the username used by user to login
	Email          string   `json:"email"`          //Alternative login identifier if provided
...
func (ar *AuthRouter) HandleUserCreate(w http.ResponseWriter, r *http.Request) {
	...
	useGroupPolicy, _ := utils.PostBool(r, "useGroupPolicy")
	user.UseGroupPolicy = useGroupPolicy
	if useGroupPolicy {
		groupId := strings.TrimSpace(r.FormValue("groupId"))
		if groupId != "" {
			// Validate that the group policy exists
			if _, err := ar.GetGroupPolicyByID(groupId); err != nil {
...
func (ar *AuthRouter) HandleUserUpdate(w http.ResponseWriter, r *http.Request) {
	...
	// Group policy settings
	if _, ok := r.Form["useGroupPolicy"]; ok {
		useGroupPolicy, _ := utils.PostBool(r, "useGroupPolicy")
		user.UseGroupPolicy = useGroupPolicy
		if useGroupPolicy {
			groupId := strings.TrimSpace(r.FormValue("groupId"))
...
func (ar *AuthRouter) ValidateUserAccessToHost(username, host string) bool {
	...
	// Resolve effective allowed hosts via group policy or user-level setting
	allowedHosts := ar.GetAllowedHostsForUser(user)
