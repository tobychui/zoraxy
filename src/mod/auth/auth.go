...
type AuthAgent struct {
	...
	totpLockoutUntil    time.Time
	totpMaxFailures     int
	totpLockoutDuration time.Duration

	// WebAuthn auth attempts rate limiting
	webauthnAuthMu              sync.Mutex
	webauthnAuthAttempts        map[string]int
	webauthnAuthLastReset       time.Time
	webauthnAuthMaxAttempts     int
	webauthnAuthWindowSeconds   int
	webauthnAuthIPLockouts      map[string]time.Time
	webauthnAuthLockoutDuration time.Duration
}
...
func NewAuthenticationAgent(sessionName string, key []byte, sysdb *db.Database, allowReg bool, systemLogger *logger.Logger, loginRedirectionHandler func(http.ResponseWriter, *http.Request)) *AuthAgent {
	...
	newAuthAgent := AuthAgent{
		...
		totpMaxFailures:     5,
		totpLockoutDuration: 1 * time.Hour,

		webauthnAuthMaxAttempts:     5,
...
func (a *AuthAgent) checkTOTPRateLimit() error {
	a.totpMu.Lock()
	defer a.totpMu.Unlock()

	if !a.totpLockoutUntil.IsZero() && time.Now().Before(a.totpLockoutUntil) {
		return fmt.Errorf("Too many failed TOTP verification attempts. Try again later or use different authentication method.")
	}

	if !a.totpLastFailure.IsZero() && time.Now().After(a.totpLastFailure.Add(1*time.Minute)) {
		a.totpFailures = 0
	}
...
func (a *AuthAgent) recordTOTPFailure(username string) {
	a.totpMu.Lock()
	defer a.totpMu.Unlock()

	a.totpFailures++
	a.totpLastFailure = time.Now()

	a.Logger.PrintAndLog(
		"auth",
		fmt.Sprintf(
			"Invalid TOTP code from %s. failures: %d, last fail: %s.",
			username, a.totpFailures, a.totpLastFailure),
		nil)

	if a.totpFailures >= a.totpMaxFailures {
		a.totpLockoutUntil = time.Now().Add(a.totpLockoutDuration)
		a.Logger.PrintAndLog("auth", "TOTP rate-limiter engaged lockout until "+a.totpLockoutUntil.Format(time.DateTime), nil)
	}
}

func (a *AuthAgent) checkWebAuthnAuthBeginRateLimit(clientIP string) error {
	a.webauthnAuthMu.Lock()
	defer a.webauthnAuthMu.Unlock()

	if lockoutUntil, exists := a.webauthnAuthIPLockouts[clientIP]; exists {
		if !lockoutUntil.IsZero() && time.Now().Before(lockoutUntil) {
			return fmt.Errorf("Too many WebAuthn authentication attempts. Try again later or use different authentication method.")
		}
		delete(a.webauthnAuthIPLockouts, clientIP)
	}

	webAuthAuthenticationWindow := time.Duration(a.webauthnAuthWindowSeconds) * time.Second
	if a.webauthnAuthLastReset.IsZero() || time.Now().After(a.webauthnAuthLastReset.Add(webAuthAuthenticationWindow)) {
		a.webauthnAuthAttempts = make(map[string]int)
		a.webauthnAuthLastReset = time.Now()
	}

	if a.webauthnAuthAttempts[clientIP] >= a.webauthnAuthMaxAttempts {
		return fmt.Errorf("Too many WebAuthn authentication attempts. Try again later or use different authentication method.")
	}
...
func (a *AuthAgent) recordWebAuthnAuthAttempt(clientIP string) {
	a.webauthnAuthMu.Lock()
	defer a.webauthnAuthMu.Unlock()

	a.webauthnAuthAttempts[clientIP]++
	if a.webauthnAuthAttempts[clientIP] >= a.webauthnAuthMaxAttempts {
		a.Logger.PrintAndLog("auth", "WebAuthn rate-limit hit by "+clientIP+", locking out for 1 hour", nil)
		a.webauthnAuthIPLockouts[clientIP] = time.Now().Add(a.webauthnAuthLockoutDuration)
	}
}
...
func (a *AuthAgent) HandleTOTPVerify(w http.ResponseWriter, r *http.Request) {
	if err := a.checkTOTPRateLimit(); err != nil {
		utils.SendErrorResponse(w, err.Error())
		return
	}
...
func (a *AuthAgent) HandleWebAuthnAuthBegin(w http.ResponseWriter, r *http.Request) {
	...
	// because GetRequesterIPUntrusted would return the reverse proxy's IP instead of the client's.
	//
	// A possible fix could be something like:
	//
	// 	if netutils.IsPrivateIP(clientIP) {
	// 		clientIP = netutils.GetRequesterIP(r)
	// 	}
	//
	// Or similar but only for a list of trusted reverse proxy IPs.
	//
	// However, source addresses (both MAC and IP) on a LAN cannot be fully trusted,
	// as they could be forged if packets are sent directly to zoraxy without passing
	// through a subnet router maintaining proper ARP tables.
	//
	// Possible workarounds might be a CAPTCHA or combining the IP with a session token
	// for limiting, but these are out of scope for this PR.
	//
	// For now, I'm leaving this as is, since it does not directly compromise the security of the WebAuthn login flow.
	clientIP := netutils.GetRequesterIPUntrusted(r)
	if clientIP == "" {
		utils.SendErrorResponse(w, "unable to determine client IP")
		return
	}

	if err := a.checkWebAuthnAuthBeginRateLimit(clientIP); err != nil {
		utils.SendErrorResponse(w, err.Error())
		return
	}

	// Record the attempt
	a.recordWebAuthnAuthAttempt(clientIP)
